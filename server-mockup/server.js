...
async function handleStartMediasoup() {
  ...
  console.log(
    '[handleStartMediasoup] mediasoup worker created [pid:%d]',
    worker.pid,
  );
  ...
  console.log('[handleStartMediasoup] mediasoup router created');
  log.trace(
...
async function handleWebrtcSendStart() {
  ...
  console.log(
    '[handleWebrtcSendStart] mediasoup WebRTC SEND transport created',
  );
...
async function handleWebrtcSendConnect(dtlsParams) {
  console.log(
    '[handleWebrtcSendConnect] iceCandidates: ',
  ...
  console.log(
    '[handleWebrtcSendConnect] mediasoup WebRTC SEND transport connected',
  );
}
...
async function handleWebrtcSendConsume(rtpCaps) {
  if (!videoProducer) {
    log.error(
      "[handleWebrtcSendConsume] BUG: The videoProducer doesn't exist!",
    );
    process.exit(1);
  }
...
async function handleStartVideoRtpProducer(enableSrtp) {
  ...
  if (enableSrtp) {
    ...
    console.log(
      '[startVideoRtpProducer] mediasoup RTP RECV producer connected',
    );
  }
}
